...
async def help(ctx: commands.Context):
    ...
    em.add_field(name = "<:mod:907620365914755082> Miscellaneous <:mod:907620365914755082>", value = "generate, pet, memes, youtube, ping, weather, slap, snipe, quote, cleardm, suggest, report, wsay, avatar, userinfo, serverinfo, timer, poll, announce, servericon, id, membercount, emojiinfo", inline = False)
...
@bot.slash_command(name = "generate", description = "Generate an image using AI")
async def generate(interaction: Interaction, *, prompt: str):
    ETA = int(time.time() + 60)
    msg = await interaction.send(f"Just a moment... ETA : <t:{ETA}:R>")
    generator = Craiyon()
    result = generator.generate(prompt)
    images = result.images
    ...
    for i in images:
        image = BytesIO(base64.decodebytes(i.encode("utf-8")))
        return await interaction.edit_original_message(content = "Content generated by **craiyon.com**", file = nextcord.File(image, "craiyon.png"))
